...
![作成した独自ルールがAnalyzeToolに追加されている](/images/article/categories/developGame/3/AnalyzeTool_AddCustomRule.png)

他のルール同様、「Analyze Selected Rules」で分析の実行、「Fix Selected Rules」で修正の実行ができます。

▼ 分析の実行イメージ  
![分析の実行イメージ](/images/article/categories/developGame/3/AnalyzeImage.png)
