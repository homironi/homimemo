...
まず、コメント通りですがどうもここに入ってくるときは末尾の`/`ありに統一してリダイレクトする設定していてもまだリダイレクトが行われていないときに通りました。  
そのため、末尾に`/`があってもなくても引っかかるよう調整して判定しています。

その後対象パスの場合は、対応するパスへリダイレクトさせます。
...
今回は静的アセットがない⇒そもそも存在しないURLのリクエストなです。  
...
<TextBlock blockType="question" title="例外のキャッチをしないと？">
  ...
  この場合、無効なURLにアクセスすると「例外が発生している」という旨のCloudFlare提供画面が表示されます。  
  明らかに以異常とわかるので、404を返すようにしています。
</TextBlock>
