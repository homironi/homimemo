...
1. リポジトリ内で、上部メニューの「Actions」をクリック  
   ![GitHubのActions](/images/article/categories/developOther/0/GitHub_Actions.png)
1. ワークフローがまだ設定されていない場合は、「Set up a workflow yourself」を選択。  
   ![Set up a workflow yourself](/images/article/categories/developOther/0/GitHub_Actions_setupWorkflow.png)  
   既存のワークフローがある場合は、「New workflow」ボタンをクリック  
   ![New workflow](/images/article/categories/developOther/0/GitHub_Actions_newWorkflow.png)
1. エディタが開いたら、そこに後述する`deployMain.yaml`の内容を貼り付け
1. エディタでコードを貼り付けたら、「Commit changes...」をクリック  
   ![workFlowCode](/images/article/categories/developOther/0/GitHub_workflowCode.png)
1. 適切なコミットメッセージを入力し、「Commit changes」をクリック  
   ![commit](/images/article/categories/developOther/0/GitHub_Actions_commit.png)
...
1. リポジトリ内で、上部メニューの「Settings」をクリック  
   ![Settings](/images/article/categories/developOther/0/GitHub_repo.png)
1. 「secrets and variables」＞「Actions」を選択  
   ![secretsAndVariables](/images/article/categories/developOther/0/GitHub_secret.png)
1. 「New repository secret」を選択  
   ![newVariable](/images/article/categories/developOther/0/GitHub_secret_newVariable.png)
...
1. 「Add secret」を選択してシークレット変数を追加  
   ![add secret](/images/article/categories/developOther/0/GitHub_secret_add.png)
...
2. **サーバー ID**が表示されていると思うので、それを確認  
   ...
   ![serverPane](/images/article/categories/developOther/0/Xserver_panel.png)
3. 続いて「サーバー情報」をクリック
4. サーバー情報のページで、**ホスト名**と**ホームディレクトリ**を確認  
   それぞれコードの、`svhoge`→ ホスト名、`/home/hogegege`→ ホームディレクトリを置き換え  
   ![serverInfo](/images/article/categories/developOther/0/Xserver_serverInfo.png){/*width="50%"*/}
...
![result](/images/article/categories/developOther/0/GitHub_Actions_result.png)
...
![SSH設定](/images/article/categories/developOther/0/SSHSettings.png)
