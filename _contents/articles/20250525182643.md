...
1. Google 日本語入力のプロパティを開く  
   ...
   ![IME のアイコン](/images/article/categories/memo/1/IME.png)
2. 開いたプロパティウィンドウの「一般」タブを開く  
   最初に開かれているタブです。
3. 「キー設定」＞「キー設定の選択」の「編集…」をクリック  
   ![Google 日本語入力プロパティウィンドウ](/images/article/categories/memo/1/propertyWindow.png)
...
5. 「入力キー」`Eisu` （CapsLock キー）の設定を `Hankaku/Zenkaku` と同じにする  
   ...
   |直接入力|IME を**有**効化|
    ...
    ![Google 日本語入力キー設定](/images/article/categories/memo/1/keySettings.png)
...
![インポートエクスポートができる](/images/article/categories/memo/1/importExport.png){/*width=50%*/}
