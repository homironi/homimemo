...
- [x] 個人サイトを作る際にどんな技術を使って作成しているか知ることができます！
- [x] Next.jsでSSGをする方法がわかります！
...
Next.jsでSSG→Cloudflare Workersでデプロイしています。

<TextBlock blockType="info">
エックスサーバー → [Cloudflare Workers](https://www.cloudflare.com/ja-jp/developer-platform/products/workers/)に変更したことで、エックスサーバーの契約終了後はドメイン代のみになる予定です！  
...
DecapCMSを入れた理由は記事の記事本文以外のデータを楽に設定したかったからです。  
11tyの頃と変わらず、マークダウンファイルの先頭の「FrontMatter」というデータを使って記事の本文以外のデータを管理しています。  

FrontMatterは以下のような`---`で挟まれた部分のことです。
...
このFrontMatterを11tyのときは手入力していたのですが、「更新日時はなんて名前だっけ」ということが多く、以前の記事からコピペしてくることが多かったのです。  
...
マークダウンファイルはルーティングに使用したくなかった＆CMSからデータが変更しやすい場所に置きたかったので、`next-mdx-remote`と`gray-matter`でparseしています。  
型安全だいすきなので、`valibot`でparseした記事のメタ情報の検証をしています。
...
ちょこちょこ細かいところはまた別途記事にできたらいいなと思っています。  
ブランチは以下のように使い分けています。
